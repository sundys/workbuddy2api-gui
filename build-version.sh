...
#   · 在 tag 上（如 v1.2.1）         -> 1.2.1
#   · tag 之后有新提交（v1.2.1-3-gabc）-> 1.2.1+3.gabc（标记"未发布"的构建）
#   · 没有任何 tag                    -> dev
# 去掉开头的 v 前缀，与镜像标签约定一致（CI 打的 tag 是 v1.2.1，镜像标签是 1.2.1）。
#
...
if [[ -z "$DESCRIBE" ]]; then
  ...
  # v1.2.1 -> 1.2.1 ；v1.2.1-3-gabc1234 -> 1.2.1+3.gabc1234
  # 去掉 v 前缀；首个 - 换成 +（版本号附加段），第二个 - 换成 .（hash 分隔）。
